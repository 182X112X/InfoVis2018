function main()
{
    ...
    {
        var n = cmap.length;
        //var index = ((S0 - S_min) * n)/(S_max - S_min);

        var id = faces[i];
        var S0 = scalars[ id[0] ];
        var S1 = scalars[ id[1] ];
        var S2 = scalars[ id[2] ];
        var C0 = new THREE.Color().setHex( cmap[ Math.round(((S0 - 0.1)*(n-1))/(0.8 - 0.1)) ][1] );
        var C1 = new THREE.Color().setHex( cmap[ Math.round(((S1 - 0.1)*(n-1))/(0.8 - 0.1)) ][1] );
        var C2 = new THREE.Color().setHex( cmap[ Math.round(((S2 - 0.1)*(n-1))/(0.8 - 0.1)) ][1] );
